...
class Picarx(object):
    ...
    def __init__(self):
        ...
        # TODO: replace these guesses with decent numbers.
        self.width = 1
        self.length = 1.3
    ...
    def forward(self,speed):
        ...
        if current_angle != 0:
            ...
            speed_ratio = 1 + np.tan(current_angle*np.pi/180) * self.width / self.length
            if current_angle > 0:
                # Turning left, increase right speed
                self.set_motor_speed(1, speed)
                self.set_motor_speed(2, -speed * speed_ratio)
            else:
                # Turning right, increase left speed
                self.set_motor_speed(1, speed * speed_ratio)
                self.set_motor_speed(2, -speed)
        else:
            self.set_motor_speed(1, speed)
            self.set_motor_speed(2, -1*speed)                  
