...
class PerceptionStuff:
    ...
    def get_x_y_r(self, area_max_contour, img_h, img_w):
    ...
    def do_all(self, img):
        global color_range, __target_color
        img_copy = img.copy()
        self.add_lines(img)
        frame_lab = self.get_formatted(img_copy)

        if __target_color in color_range:
            frame_mask = self.get_mask(frame_lab)
            dilated = self.erode_and_dilate_mask(frame_mask)
